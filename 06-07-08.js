  ...
  class Profesor {
    ...
    getCursos() {
      ...
      // persona.getCursos() // retorna ['javascript 1', 'javascript 2'];
       return this.cursos.map((item) => item)
    }
    ...
    getUltimoCertificado() {
      ...
      let certificados = this.certificados;
      let certificadoReciente = [];

      for(let i = 0; i < certificados.length; i++) {
        for(let j = 1; j < certificados.length; j++) {
          if(certificados[i].anio > certificados[j].anio){
            certificadoReciente.push(certificados[i])
            break
          }else{
            certificadoReciente.push(certificados[j])
            break
          }
        }
      }
      return certificadoReciente[0]
